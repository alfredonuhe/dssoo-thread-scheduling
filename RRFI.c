...
También controla la ascensión de los procesos de baja prioridad a la cola de alta cuando han esperado un número de ticks marcado por la variable STARVATION.
*/

void timer_interrupt(int sig)
{
  /* Control de la ascensión de los procesos de baja prioridad a la cola de alta*/

  /*El contador de hungry se activa cuando la cola de baja prioridad no este vacia y la de alta no lo esté o haya un hilo de alta prioridad ejecutandose*/
  if(queue_empty(lowq)==0 && (queue_empty(highq)==0 || running->priority==1)){     
         hungry++;

          /* Si hungry llega al valor de STARVATION se mueven todos los procesos que haya en ese momento en la cola de listos de prioridad 
             baja a la cola de prioridad alta y se reinicia la variable hungry*/
         if (hungry==STARVATION){
            while(!queue_empty(lowq)){
                 TCB *t= dequeue(lowq);
		 printf("*** THREAD %i PROMOTED TO HIGH PRIORITY QUEUE\n", t->tid);
                 enqueue(highq, t);
            }
            hungry=0;
         }

  }else{   //Si no se cumple la condición anterior, la variable hungry se reinicia
        hungry=0;
  }

  /*  Control de las rodajas de los procesos de baja prioridad  */
...
void activator(TCB* next){
	...
	}else if(prev->priority==0 && next->priority==1 && prev->ticks!=0){
                disable_interrupt();
                enqueue(lowq, prev);
		enable_interrupt();
